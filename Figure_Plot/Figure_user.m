...
for u = 1:numel(num_users)
    ...
    for rng = num_start:num_end
        load(['./result/proposed_choosebetter_4.65GHz_3circle/',num2str(num_users(u)),'user/result_d',num2str(distance),'_rng',num2str(rng),'.mat'],'cumulative_throughput')
   disp(cumulative_throughput(:, :, 1));     
    ...
    % % proposed_p2
    for rng = num_start:num_end
        load(['./result/conventional_4.65GHz_3circle/',num2str(num_users(u)),'user/result_d',num2str(distance),'_rng',num2str(rng),'.mat'],'cumulative_throughput')
